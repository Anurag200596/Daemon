...
export const validateInternalKey = (key: string) => {
    ...
    if (!internalKey) throw new Error("Convex Internal key is not configured");
...
export const getProcessingMessage = query({
    ...
    handler: async (ctx, args) => {
        validateInternalKey(args.internalKey)
        return await ctx.db.query("messages").withIndex("by_project_status", (q) =>
               q
...
export const getRecentMessages = query({
  ...
  export const createBinaryFile = mutation({
    ...
    handler: async(ctx,args) => {
      ...
      const files = await ctx.db.query("files").withIndex("by_project_parent", (q) => q
      .eq("projectId",args.projectId)
      .eq("parentId",args.parentId))
      .collect()

      const existing = files.find((file )=> file.name === args.name)

      if(existing) throw new Error("File with the given name already exists");
  ...
  export const getProjectFileWithUrl = query({
    args:{
      projectId : v.id("projects"),
      internalKey : v.string(),
    },
    handler: async(ctx,args) => {
      ...
      const files = await ctx.db.query("files").withIndex("by_projectId", (q) => q
      .eq("projectId",args.projectId)).collect()
