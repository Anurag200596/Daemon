...
export const  getFiles = query({
    ...
    handler: async(ctx,args) => {
        ...
        if(project.ownerId !== identity.subject) throw new Error("Unauthorized acces of files")

        return ctx.db.query("files").withIndex("by_projectId", (q) => q.eq("projectId" , args.projectId)).collect() 
...
export const getFolderContents = query({
 ...
 export const getFilePath = query({
  ...
  handler : async(ctx,args) => {
    ...
    if(!identity) throw new Error("Unauthorized acces to the files")
      ...
      if(!file ) return null;
        ...
        if(project?.ownerId !== identity.subject) throw new Error("Unauthorized acces to other's Project")
          ...
          while(currentId){
            ...
            if(!file) break;
            path.unshift({_id: currentId,name : file.name})
